...
app.post('/api/mpesa/withdraw', async (req, res) => {
    ...
    try {
        ...
        // IDEMPOTENCY KEY
        //
        // Frontend must generate a unique key for every
        // withdrawal attempt and reuse it if the request
        // is retried.
        // =====================================================
        ...
        // Avoid excessive floating-point precision.
        ...
        try {
            ...
            // CRITICAL:
            //
            // A timeout does NOT prove that Safaricom failed.
            //
            // Never refund here.
            // =================================================

            console.error(
                "❌ M-PESA SUBMISSION ERROR:",
        ...
        if (!payoutId) {
            ...
            switch (
                ...
                case "PAYOUT_ALREADY_PROCESSING":
        ...
        // =====================================================
        // UNKNOWN ERROR AFTER RESERVATION
        //
        // NEVER BLINDLY REFUND.
        // =====================================================
        ...
        return res.status(202).json({
            ...
            message:
                "Withdrawal is being reconciled.",
    ...
    try {
        ...
        await session.withTransaction(
            async () => {
                ...
                // Only definitively failed payouts can be
...
app.post('/api/mpesa/b2c/result', async (req, res) => {
    ...
    try {
        ...
        if (!conversationId) {
            ...
            console.error(
                "⚠️ B2C callback missing ConversationID"
            );
            ...
            try {
                ...
                await session.withTransaction(
                    async () => {
                        ...
                        // -----------------------------------------
                        // Already completed / refunded / failed
                        // -----------------------------------------
                        ...
                        //
                        // IMPORTANT:
                        // The actual wallet debit happened during
                        // WITHDRAWAL_RESERVE.
                        //
                        // This is a settlement/audit event,
                        // not another wallet debit.
                        // -----------------------------------------
            ...
            await session.withTransaction(
                async () => {
                    ...
                    if (!user) {
                        ...
                        throw new Error(
                            "USER_NOT_FOUND_DURING_CALLBACK_REFUND"
                        );
                    }
                    ...
                    lockedPayout.refundReason =
                        result.ResultDesc;

                    lockedPayout.refundedAt =
                        new Date();

                    lockedPayout.ledgerRefundReference =
                        reference;
...
app.post('/api/products/:id/purchase', async (req, res) => {
  ...
  try {
    ...
    const requestedCurrency =
      String(currency || baseCurrency).toUpperCase();
    ...
    /*
     * If GeoSmart supplied an amount,
     * that amount is the localized customer price.
     *
     * DO NOT recalculate or replace it here.
     */
    ...
    const method =
      String(paymentMethod || 'mobile_money').toLowerCase();
    ...
    if (method === 'mobile_money') {
       ...
       *
       * GeoSmart pricing remains the customer's
       * display price. Conversion to KES happens
       * ONLY at the payment gateway stage.
       */
      ...
      if (requestedCurrency === 'KES') {

        mpesaAmount = finalAmount;

      } else {

        const fxResponse = await fetch(
          `https://api.frankfurter.app/latest?amount=${finalAmount}&from=${requestedCurrency}&to=KES`
        );

        if (!fxResponse.ok) {
          throw new Error(
            `Unable to convert ${requestedCurrency} to KES`
          );
        }
        ...
        if (
          !Number.isFinite(mpesaAmount) ||
          mpesaAmount <= 0
        ) {
          throw new Error(
            'Unable to calculate KES payment amount'
          );
        }
      }

      mpesaAmount =
        Math.round(mpesaAmount);

      const result =
        await triggerUniversalPush(
          phone,
          mpesaAmount,
          productId,
      ...
      return res.json({
        ...
        // Amount actually sent to M-Pesa
        amount:
          mpesaAmount,

        // Original GeoSmart customer price
        displayCurrency:
          requestedCurrency,
    ...
    if (method === 'stripe') {
      ...
      if (!stripeCurrency) {
        return res.status(400).json({
          error:
            'GeoSmart currency is required for Stripe'
        });
      }

      /*
       * GeoSmart has already supplied the
       * localized price.
       *
       * Stripe receives that exact price.
       */

      const stripeDisplayAmount =
        Number(finalAmount.toFixed(2));

      if (
        !Number.isFinite(stripeDisplayAmount) ||
        stripeDisplayAmount <= 0
      ) {
        return res.status(400).json({
          error:
    ...
    console.error(
      'GeoSmart product purchase error:',
      error
    );
